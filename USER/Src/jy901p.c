...
static int16_t JY901P_Read16BitReg(uint8_t reg_addr)
{
    ...
    if (HAL_I2C_Mem_Read(&hi2c1, JY901P_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, temp_buf, 2, 100) != HAL_OK)
    {
        return 0;
    }
...
uint8_t jy901p_i2c1_dma_rx_buf[18] = {0};

// 新增：读取I2C1 DMA接收的当前状态与已接收数据
// pRxData: 输出缓冲区，用于存放DMA接收到的数据
// max_len: 输出缓冲区的最大长度
// 返回值：实际已接收的字节数
uint32_t JY901P_ReadI2C1_DMA_Data(uint8_t *pRxData, uint32_t max_len)
{
    // 1. 获取DMA剩余未传输的数据量
    ...
    if (pRxData != NULL && recv_len > 0)
    {
        ...
        memcpy(pRxData, jy901p_i2c1_dma_rx_buf, copy_len);
    }
